...
while ( my $row = $csv->getline(@lines) ) {
    ...
    pop @hh;
    my $portref =
"<a href='http://www.portcullis.parliament.uk/CalmView/Record.aspx?src=CalmView.Catalog&id="
      . join( "%2f", @hh )
      . "'>Portcullis</a>";
    ...
    my $YJSON = get(
'http://query.yahooapis.com/v1/public/yql?q=SELECT%20*%20FROM%20geo.placefinder%20WHERE%20name%3D%22'
          . $nice_first
          . '%22%20and%20locale%3D%22GB%22%20%7C%20truncate(count%3D1)&format=json'
    );
    my $YResponse = decode_json $YJSON;

    if ( $YResponse->{query}->{results}->{Result}->{countrycode} eq "GB" ) {
